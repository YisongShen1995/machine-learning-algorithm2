...
class linear_layer:
    ...
    def backward(self, X, grad):
        ...
        self.gradient['b'] = np.dot(temp.T,grad)
...
class relu:
    ...
    def forward(self, X):
        ...
        """
        T = np.zeros(X.shape)
        forward_output=np.maximum(X,T)
        self.mask = np.sign(forward_output)
